...
resource vsphere_virtual_machine "vm_cloudInitTest" {
  name             = "${var.cloudInitTest_srv_config["vmname"]}"
  folder           = "${var.cloudInitTest_srv_config["vfolder"]}"
    ...
    "guestinfo.metadata"          = base64encode(templatefile("${path.module}/../../srvTemplate/metadata/metadata1nicDyn.tftpl",
                                    {
                                      ...
                                      gw="${var.cloudInitTest_srv_config["gw"]}",
                                      dns= "${split(",", var.cloudInitTest_srv_config["dns"])}"
                                    }))
    "guestinfo.metadata.encoding" = "base64"
    "guestinfo.userdata" = data.template_cloudinit_config.cloudInitTest.rendered
    "guestinfo.userdata.encoding" = "gzip+base64"
  }
  
}

data template_cloudinit_config "cloudInitTest" {
  gzip          = true
  base64_encode = true
