...
resource vsphere_virtual_machine "keycloak_01" {
  name             = "${var.keycloak_srv_config["vmname"]}"
  folder           = "${var.keycloak_srv_config["vfolder"]}"
    ...
    "guestinfo.metadata"          = base64encode(templatefile("${path.module}/../../srvTemplate/metadata/metadata1nicDyn.tftpl",
                                    {
                                      ...
                                      gw="${var.keycloak_srv_config["gw"]}",
                                      dns= "${split(",", var.keycloak_srv_config["dns"])}"
                                    }))
    "guestinfo.metadata.encoding" = "base64"
    "guestinfo.userdata" = data.template_cloudinit_config.keycloak_01.rendered
    "guestinfo.userdata.encoding" = "gzip+base64"
  }
  
}

data template_cloudinit_config "keycloak_01" {
  gzip          = true
  base64_encode = true
  ...
  part {
    ...
    content  = templatefile("${path.module}/../../srvTemplate/userdata.split/30.keycloak.core.tftpl",
    {
      ...
      sysgroup="${var.keycloak_srv_config["sysgroup"]}",
      dbname="${var.keycloak_srv_config["dbname"]}",
      dblogin="${var.keycloak_srv_config["dblogin"]}",
      dbpassword="${var.keycloak_srv_config["dbpassword"]}",
      kcadmin="${var.keycloak_srv_config["kcadmin"]}",
      kcadminpassword="${var.keycloak_srv_config["kcadminpassword"]}"
    })
  }
  part {
    content_type = "text/x-shellscript"
